...
def run_chunk_issue_scan(cfg: OpenRouterConfig, chunks: List[Tuple[int, int, str]], sleep_sec: float = 0.0) -> List[Dict[str, Any]]:
    ...
    for idx, (sp, ep, ctext) in enumerate(chunks, start=1):
        print(f"  Chunk {idx}/{total} (Pages {sp}-{ep})...")
        ...
        try:
            ...
            # If parsing failed and returned dict with error, log it but keep going
            if "_error" in obj:
                print(f"    Warning: JSON parse error in chunk {idx}: {obj.get('_error')}")
            
            # enforce page range if missing or malformed
            if not isinstance(obj, dict):
                obj = {"issues": [], "_raw": str(obj)}
            ...
            results.append({"pages": {"start": sp, "end": ep}, "issues": [], "_error": str(e)})
...
def run_merge_dedup(cfg: OpenRouterConfig, issue_jsons: List[Dict[str, Any]]) -> Dict[str, Any]:
    print("Merging and deduplicating issues...")
    # Prepare list for prompt - remove raw text to save tokens if needed, but keeping for context
    # We might want to filter out empty issues to save context window
    filtered_jsons = [j for j in issue_jsons if j.get("issues")]
    
    if not filtered_jsons:
        return {"issues": []}

    user = MERGE_DEDUP_USER.format(json_list=json.dumps(filtered_jsons, ensure_ascii=False))
    messages = [
...
def run_gpt_review(
    ...
    if len(full_text) <= max_input_chars:
        print(f"Paper fits in context ({len(full_text)} chars <= {max_input_chars}). Using full text mode.")
        ...
        print(f"Paper too long ({len(full_text)} chars > {max_input_chars}). Using chunk-scan mode.")
    ...
    print(f"[OK] GPT review completed. Output written to: {out_file}")
